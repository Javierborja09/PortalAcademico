...
const AulaSesionParticipantes = ({ cursoId }) => {
  const { participantes, usuarioNombreActual, totalConectados } =
    useParticipantes(cursoId);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/50">
        ...
        {participantes.length === 0 ? (
          ...
          participantes.map((p, index) => (
            <div
              key={index}
              className="flex items-center gap-3 p-3 bg-white rounded-2xl border border-slate-100 animate-fadeIn hover:border-indigo-200 transition-all"
            >
