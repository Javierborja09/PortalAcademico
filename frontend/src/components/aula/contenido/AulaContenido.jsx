...
const AulaContenido = ({
  ...
  const getFileIcon = (filename) => {
    ...
    if (ext.match(/\.(jpg|jpeg|png|webp|gif)$/))
      return <FileText className="text-emerald-500" size={14} />;
    if (ext.match(/\.(zip|rar|7z)$/))
      return <FileArchive className="text-amber-500" size={14} />;
    if (ext.match(/\.(txt|doc|docx)$/))
  ...
  return (
    <div className="space-y-2 animate-fadeIn px-1 md:px-0">
      {unidades.map((unidad) => (
        <div
          key={unidad.idUnidad}
          className="border border-slate-100 rounded-[1.2rem] md:rounded-[2rem] overflow-hidden bg-white shadow-sm"
        >
          {/* Header Unidad */}
          <div
          ...
          >
            <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
              ...
              <h3 className="font-black text-slate-800 uppercase text-[10px] md:text-xs truncate tracking-tight">
                {unidad.tituloUnidad}
              </h3>
            </div>
            {rol === "docente" && (
              ...
              >
                <button
                  ...
                  className="p-1.5 bg-white text-slate-400 hover:text-blue-600 rounded-lg border border-slate-100"
                >
                  <Edit3 size={12} />
                </button>
                <button
                  onClick={() => onDelete("unidad", unidad.idUnidad)}
                  className="p-1.5 bg-white text-slate-400 hover:text-red-500 rounded-lg border border-slate-100"
                >
                  <Trash2 size={12} />
                </button>
              </div>
          ...
          {expandedUnidades[unidad.idUnidad] && (
            <div className="px-2 pb-3 md:px-4 md:pb-4 space-y-1.5 animate-slideDown bg-slate-50/10">
              {unidad.temas?.map((tema) => (
                <div
                  key={tema.id_tema}
                  className="rounded-[1rem] border border-slate-100 overflow-hidden bg-white shadow-sm"
                >
                  <div
                  ...
                  >
                    <div className="flex items-center gap-2 overflow-hidden flex-1">
                      <div
                        className={`shrink-0 w-6 h-6 md:w-8 md:h-8 rounded-lg flex items-center justify-center transition-all ${
                          expandedTemas[tema.id_tema]
                      ...
                      <div className="overflow-hidden">
                        <h4 className="text-[9px] md:text-[10px] font-black text-slate-600 uppercase truncate">
                          {tema.tituloTema}
                        </h4>
                    ...
                    {rol === "docente" && (
                      ...
                      >
                        <button
                          onClick={() => onUploadFile(tema.id_tema)}
                          className="p-1 text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100"
                        >
                          <UploadCloud size={13} />
                        </button>
                        <button
                          onClick={() => onEditTema(unidad.idUnidad, tema)}
                          className="p-1 text-slate-400 hover:text-blue-600"
                        >
                          <Edit3 size={13} />
                        </button>
                        <button
                          onClick={() => onDelete("tema", tema.id_tema)}
                    ...
                    <div className="px-2 pb-2 animate-slideDown border-t border-slate-50 bg-slate-50/20">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 mt-2">
                        {tema.documentos?.length > 0 ? (
                          tema.documentos.map((doc) => {
                            ...
                            return (
                              <div
                                ...
                                className="flex items-center justify-between p-2 bg-white rounded-lg border border-slate-100 group transition-all gap-2"
                              >
                                {/* Lado izquierdo: Icono y Nombre */}
                                <div
                                  className={`flex items-center gap-2 overflow-hidden flex-1 ${
                                    isPreviable ? "cursor-pointer" : ""
                                  }`}
                                  onClick={() => isPreviable && onPreview(doc)}
                                >
                                ...
                                <div className="flex items-center gap-0.5 shrink-0">
                                  ...
                                  <a
                                    href={`http://localhost:8080${doc.rutaArchivo}`}
                                    download
                                    className="p-1 text-slate-400 hover:text-blue-600 transition-colors"
                                    title="Descargar"
                                  >
                                  ...
                                  {rol === "docente" && (
                                    <button
                                      onClick={() =>
                                        onDelete("documento", doc.id_documento)
                                      }
                          ...
                          <p className="col-span-full text-center py-2 text-[8px] text-slate-300 font-bold uppercase tracking-widest italic">
