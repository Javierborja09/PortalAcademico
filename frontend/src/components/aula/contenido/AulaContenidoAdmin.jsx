...
import { 
    Plus, Edit3, Trash2, Layout, FileText, 
    UploadCloud, X 
} from "lucide-react";

const AulaContenidoAdmin = ({ unidades, openUnidadModal, openTemaModal, openUploadModal, handleDelete }) => {
    return (
        <div className="space-y-4 animate-fadeIn">
            ...
            <div className="space-y-3">
                {unidades.map((u) => (
                    <div key={u.idUnidad} className="bg-slate-50/50 border border-slate-100 rounded-[1.5rem] p-4 md:p-5">
                        {/* Fila de Unidad */}
                        <div className="flex items-center justify-between mb-4 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            <div className="flex gap-1">
                                <button onClick={() => openUnidadModal(u)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                                    <Edit3 size={15} />
                                </button>
                                <button onClick={() => handleDelete("unidad", u.idUnidad)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                                    <Trash2 size={15} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="pl-4 md:pl-8 space-y-2">
                            {u.temas?.map((t) => (
                                <div key={t.id_tema} className="bg-white p-3 md:p-4 rounded-xl border border-slate-100 shadow-xs">
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <div className="flex gap-1 shrink-0">
                                            <button onClick={() => openUploadModal(t.id_tema)} title="Subir Archivo" className="p-1.5 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-all">
                                                <UploadCloud size={14}/>
                                            </button>
                                            <button onClick={() => openTemaModal(u.idUnidad, t)} className="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg transition-all">
                                                <Edit3 size={14} />
                                            </button>
                                            <button onClick={() => handleDelete("tema", t.id_tema)} className="p-1.5 text-slate-400 hover:text-red-500 rounded-lg transition-all">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    {t.documentos?.length > 0 && (
                                        <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-slate-50">
                                            {t.documentos.map(doc => (
                                                <div key={doc.id_documento} className="flex items-center gap-2 px-2 py-1 bg-slate-50 rounded-md border border-slate-100 group transition-all">
                                                    <span className="text-[8px] font-black text-slate-500 truncate max-w-[100px] uppercase">
                                                        {doc.tituloDocumento}
                                                    </span>
                                                    <button 
                                                        onClick={() => handleDelete("documento", doc.id_documento)} 
