  ...
  return (
    <div className="h-screen bg-slate-950 flex flex-col overflow-hidden text-white relative">
      ...
      {showEndModal && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center bg-slate-950/90 backdrop-blur-xl p-4">
          <div className="bg-white p-10 rounded-[3rem] max-w-sm w-full text-center shadow-2xl animate-slideUp">
            ...
            <button
              onClick={() => (window.location.href = `/aula-virtual/${id}`)}
      ...
      <header className="px-6 py-4 bg-slate-900/80 backdrop-blur-xl border-b border-white/5 flex justify-between items-center z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-blue-400 font-bold uppercase opacity-70">
              ID: {id}
            </p>
          </div>
        </div>
        <button
          ...
          className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white px-5 py-2.5 rounded-2xl transition-all font-black text-[10px] uppercase"
        >
          <LogOut size={14} />{" "}
          <span>{rol === "docente" ? "Terminar Clase" : "Salir"}</span>
        </button>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        {/* VIDEO AREA */}
        <div
          className={`flex-1 relative items-center justify-center p-8 transition-all duration-300 ${activeTab ? "hidden md:flex" : "flex"}`}
        >
          <div className="w-full h-full max-w-6xl bg-slate-900 rounded-[3rem] border-2 border-white/5 shadow-2xl flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <p className="font-black uppercase text-[10px] tracking-[0.4em] text-slate-600">
          ...
          <aside className="fixed inset-0 z-[100] md:relative md:inset-auto w-full md:w-[380px] md:border-l border-white/5 bg-slate-950 flex flex-col animate-slideInRight">
            <div className="flex items-center justify-between p-4 bg-slate-900 md:hidden">
              <h3 className="text-xs font-black uppercase tracking-widest text-white">
                {activeTab === "chat" ? "Chat" : "Asistencia"}
              </h3>
              <button onClick={() => setActiveTab(null)} className="p-2">
                <X size={20} />
              </button>
            </div>
      ...
      <footer className="px-8 py-6 bg-slate-950 border-t border-white/5 flex justify-center items-center gap-6 z-50">
        <div className="flex gap-3 bg-slate-900/50 p-2 rounded-[2rem] border border-white/5">
          <button className="p-4 rounded-2xl bg-red-500/10 text-red-500 border border-red-500/20 active:scale-90 transition-all">
            <MicOff size={20} />
          </button>
          <button className="p-4 rounded-2xl bg-red-500/10 text-red-500 border border-red-500/20 active:scale-90 transition-all">
            <VideoOff size={20} />
        ...
        <div className="flex gap-3 bg-slate-900/50 p-2 rounded-[2rem] border border-white/5">
          <button
            ...
            className={`p-4 rounded-2xl transition-all ${activeTab === "users" ? "bg-white text-slate-900" : "text-slate-400 hover:bg-white/5"}`}
          >
            <Users size={20} />
          </button>
          <button
            onClick={() => toggleTab("chat")}
            className={`p-4 rounded-2xl transition-all ${activeTab === "chat" ? "bg-white text-slate-900" : "text-slate-400 hover:bg-white/5"}`}
          >
