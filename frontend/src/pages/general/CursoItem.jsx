...
import { 
    BookOpen, User, Settings, ArrowRight, 
    Calendar, Info, Users, Image as ImageIcon 
} from "lucide-react";
...
const CursoItem = ({ curso, rol, onRefresh }) => {
    ...
    const { 
        modals, horarios, handleOpenDetails, handleIrAAulaVirtual 
        ...
        <>
            <div className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-slate-100 hover:shadow-2xl hover:border-blue-100 hover:-translate-y-2 transition-all duration-500 flex flex-col h-full group relative overflow-hidden">
                ...
                <div className="relative w-full h-28 mb-6 overflow-hidden rounded-[1.5rem] bg-slate-100 border border-slate-100">
                    <img
                        src={curso.imagenPortada ? `${API_BASE}${curso.imagenPortada}` : null}
                        alt={curso.nombreCurso}
                        className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"
                        onError={(e) => {
                            e.target.onerror = null;
                            e.target.src = `${API_BASE}/courses/default.webp`;
                        }}
                    />
                    ...
                    <div className="absolute top-3 right-3 flex gap-2">
                        {rol === "admin" && (
                            <button onClick={() => modals.matricula.set(true)} className="p-2 bg-white/20 hover:bg-white backdrop-blur-md text-white hover:text-indigo-600 rounded-xl transition-all border border-white/10 shadow-lg">
                                <Users size={16} />
                            </button>
                        )}
                        <button onClick={handleOpenDetails} className="p-2 bg-white/20 hover:bg-white backdrop-blur-md text-white hover:text-blue-600 rounded-xl transition-all border border-white/10 shadow-lg">
                            <Info size={16} />
                        </button>
                    </div>
                </div>
                ...
                <div className="relative z-10">
                    <button 
                        onClick={() => rol === 'admin' ? modals.admin.set(true) : handleIrAAulaVirtual()}
                    ...
                    >
                        ...
                        {rol !== 'admin' && <ArrowRight size={16} className="ml-1 group-hover/btn:translate-x-1 transition-transform" />}
                    </button>
                </div>
            </div>

            {/* MODALES */}
            <CursoDetalle isOpen={modals.details.isOpen} onClose={() => modals.details.set(false)} curso={curso} horarios={horarios} />
            {rol === "admin" && (
                <>
                    <CursoAdmin isOpen={modals.admin.isOpen} onClose={() => modals.admin.set(false)} curso={curso} onSave={onRefresh} />
                    <MatriculaAdmin isOpen={modals.matricula.isOpen} onClose={() => modals.matricula.set(false)} curso={curso} />
                </>
            )}
        </>
    );
