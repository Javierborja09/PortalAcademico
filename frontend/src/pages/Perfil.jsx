    ...
    <div className="animate-fadeIn flex justify-center items-start pt-10 px-4 pb-20">
      <div className="max-w-4xl w-full grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {mensaje.texto && (
            <div className={`p-5 rounded-[2rem] border flex items-center gap-3 animate-slideUp ${mensaje.tipo === "success" ? "bg-emerald-50 border-emerald-100 text-emerald-700" : "bg-rose-50 border-rose-100 text-rose-700"}`}>
        ...
        <div className="lg:col-span-7 bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm">
          <header className="mb-10">
            ...
            <p className="text-slate-400 font-medium">Información registrada en el sistema</p>
          </header>

          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="group space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center gap-2"><User size={14} /> Nombre Completo</label>
              <input type="text" value={user.nombre} disabled className="w-full p-5 bg-slate-50 border-transparent rounded-[1.5rem] text-slate-600 font-bold cursor-not-allowed" />
            </div>

            <div className="group space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 flex items-center gap-2"><ShieldCheck size={14} /> Rango Académico</label>
              <input type="text" value={user.rol} disabled className="w-full p-5 bg-slate-50 border-transparent rounded-[1.5rem] text-slate-600 font-bold uppercase cursor-not-allowed" />
            </div>
            ...
            >
              {loading ? <><Loader2 className="animate-spin" /> Procesando</> : "Guardar Cambios"}
            </button>
          </form>
        </div>
      </div>
