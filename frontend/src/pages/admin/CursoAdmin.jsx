...
import { X, Save, Upload, Loader2, BookOpen, UserCheck, Hash } from 'lucide-react'; // Añadido Hash para el código
...
const CursoAdmin = ({ isOpen, onClose, curso = null, onSave }) => {
        ...
        if (isOpen) {
            ...
            if (curso) {
                setFormData({
                    ...
                    codigoCurso: curso.codigoCurso || '', // <--- Cargar código si existe
                    idDocente: curso.docente?.id_usuario || '',
                    fechaInicio: curso.fechaInicio,
                    fechaFin: curso.fechaFin
                });
                // Cache buster para forzar actualización de imagen antigua en el navegador
    ...
    const handleSubmit = async (e) => {
        ...
        data.append('codigoCurso', formData.codigoCurso); // <--- Enviado al backend
        ...
        try {
            ...
            if (curso) {
                ...
                // El idDocente se pasa como Query Param según tu controlador de Spring Boot
                response = await api.post(`/cursos/crear?idDocente=${formData.idDocente}`, data);
            }
            ...
            alert("Error al procesar el curso: " + (err.response?.data?.mensaje || "Problema de conexión"));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <form onSubmit={handleSubmit} className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-slideUp">
                ...
                <div className="p-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="w-full h-40 rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden relative group">
                            {preview ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-slate-400">
                                    <Upload size={32} />
                                    <span className="text-[10px] font-bold uppercase mt-2">Subir Portada (WebP)</span>
                                </div>
                            )}
                            <input 
                                type="file" 
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-slate-700"
                                value={formData.nombreCurso} onChange={e => setFormData({...formData, nombreCurso: e.target.value})} placeholder="Ej. Algoritmos I" />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <input required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-slate-700"
                                value={formData.codigoCurso} onChange={e => setFormData({...formData, codigoCurso: e.target.value})} placeholder="Ej. INF-101" />
                        </div>
                    </div>

                    {/* Docente Asignado */}
                    <div className="md:col-span-2 space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Docente Asignado</label>
                        <div className="relative">
                            ...
                            <select required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-transparent rounded-2xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all font-bold text-slate-700 appearance-none"
                                value={formData.idDocente} onChange={e => setFormData({...formData, idDocente: e.target.value})}>
                    ...
                    <div className="space-y-2">
                        ...
                        <input type="date" required className="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-700"
                ...
                <div className="p-10 pt-0 flex gap-4">
                    <button type="submit" disabled={loading} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-[1.5rem] font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-all shadow-xl shadow-blue-200">
