...
const UsuarioItem = ({ usuario }) => {
    ...
    const getRoleConfig = (rol) => {
        ...
        const configs = {
            admin: { style: "bg-purple-50 text-purple-600 border-purple-100", icon: <ShieldCheck size={12} className="mr-1.5" /> },
            docente: { style: "bg-amber-50 text-amber-600 border-amber-100", icon: <UserCog size={12} className="mr-1.5" /> },
            alumno: { style: "bg-blue-50 text-blue-600 border-blue-100", icon: <GraduationCap size={12} className="mr-1.5" /> }
        };
        ...
        <tr className="flex flex-col md:table-row border-b border-slate-50 last:border-0 hover:bg-slate-50/80 transition-all duration-300 group p-4 md:p-0">
            ...
            <td className="md:p-5 pb-3">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-sm font-black text-slate-800 leading-tight">{usuario.nombre} {usuario.apellido}</p>
            ...
            {/* COLUMNA CORREO (Se oculta en móviles muy pequeños para ahorrar espacio) */}
            <td className="md:p-5 py-2 hidden sm:table-cell">
                <div className="flex items-center gap-2 text-sm text-slate-600 font-medium italic">
                    ...
                    <span className="truncate max-w-[150px] lg:max-w-none">{usuario.correo}</span>
                </div>
            </td>
            ...
            <td className="md:p-5 pt-4 md:text-right">
                <div className="flex md:justify-end gap-2 md:opacity-0 group-hover:opacity-100 transition-all">
                    <button 
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 md:p-2.5 p-3 bg-white hover:bg-blue-50 text-slate-400 hover:text-blue-600 rounded-xl border border-slate-100 md:border-transparent hover:border-blue-100 transition-all shadow-sm md:shadow-none"
                        title="Editar"
                    >
                        <Pencil size={16} strokeWidth={2.5} />
                        <span className="md:hidden text-[10px] font-black uppercase">Editar</span>
                    </button>
                    <button 
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 md:p-2.5 p-3 bg-white hover:bg-red-50 text-slate-400 hover:text-red-500 rounded-xl border border-slate-100 md:border-transparent hover:border-red-100 transition-all shadow-sm md:shadow-none"
                        title="Eliminar"
                    >
